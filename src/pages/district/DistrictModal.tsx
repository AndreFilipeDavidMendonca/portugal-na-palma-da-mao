...
export default function DistrictModal({
  ...
  const activeBbox = useMemo(
    () => (districtFeature ? bboxFromFeature(districtFeature) : null),
    [districtFeature]
  );
  ...
  const districtNameFallback = useMemo(() => {
    return districtFeature?.properties?.name || districtFeature?.properties?.NAME || "Distrito";
  }, [districtFeature]);
  ...
  const resetPoiModalState = useCallback(() => {
    setSelectedPoi(null);
    setPoiInfo(null);
  ...
  const applyDistrictInfo = useCallback(
    ...
    [districtNameFallback, resetDistrictForm]
  );
  ...
  }, [open, districtId, resetDistrictForm, resetPoiModalState]);
  ...
  useEffect(() => {
    ...
    resetDistrictForm();
    resetPoiModalState();
  }, [open, resetDistrictForm, resetPoiModalState]);
  ...
  }, [open, districtId, applyDistrictInfo, onLoaded]);
    ...
    (async () => {
      try {
        ...
        if (e?.name !== "AbortError") {
          console.error("[DistrictModal] erro ao buscar POIs:", e);
        }
      }
    })();
  ...
  const handleCancel = useCallback(() => {
    setEditingDistrict(false);
    setDistrictError(null);
    applyDistrictInfo(districtInfo);
  ...
  return (
    <div className="district-modal theme-dark">
      ...
      {(loadingPoi || savingDistrict) && (
        <SpinnerOverlay
          open={loadingPoi || savingDistrict}
          message="A carregar…"
        />
      )}
    </div>
